...
export default function App() {
  ...
  const [dailyStudy, setDailyStudy] = useState({ date: null, seconds: 0 }); // today's accumulated study time
  ...
  const loadDailyStudy = useCallback(async (id) => {
    try {
      const res = await window.storage.get(`dailystudy:${id}`, false);
      if (res?.value) {
  ...
  const enterAppForUser = useCallback(async (user) => {
    ...
    const [p, s, ds] = await Promise.all([loadProfileData(user.id), loadStreak(user.id), loadDailyStudy(user.id)]);
    setProfile(p);
    setStreak(s);
    setDailyStudy(ds);
    setView("home");
  }, [loadProfileData, loadStreak, loadDailyStudy]);
  ...
  const saveDailyStudy = async (id, d) => {
    try {
      await window.storage.set(`dailystudy:${id}`, JSON.stringify(d), false);
    } catch (e) {}
  };
  ...
  const todaysTotalSeconds = (liveSeconds = 0) => {
    const base = dailyStudy.date === todayStr() ? dailyStudy.seconds : 0;
    return base + liveSeconds;
  };
  ...
  const startSession = async () => {
    ...
    const ping = async () => {
      ...
      const live = Math.floor((Date.now() - sessionStartRef.current) / 1000);
      checkStreakThreshold(live);
    };
    ping();
  ...
  const stopSession = async () => {
    ...
    if (wasRunning && sessionStartRef.current) {
      ...
      const today = todayStr();
      const base = dailyStudy.date === today ? dailyStudy.seconds : 0;
      const nextTotal = base + elapsed;
      const next = { date: today, seconds: nextTotal };
      setDailyStudy(next);
      await saveDailyStudy(uid, next);
