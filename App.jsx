...
export default function App() {
  const [view, setView] = useState("loading"); // loading, auth, home, subjects, chapter, room, history
  ...
  const enterAppForUser = useCallback(async (user) => {
    ...
    const [p, s, secs] = await Promise.all([loadProfileData(user.id), loadStreak(user.id), loadDailySeconds(user.id)]);
    setProfile(p);
    setStreak(s);
    setDailySeconds(secs);
    const resumed = await resumeIfActive(user.id, displayName, secs);
    if (!resumed) setView("home");
  }, [loadProfileData, loadStreak, loadDailySeconds, resumeIfActive]);
  ...
  const stopSession = async () => {
    ...
    if (wasRunning && sessionStartRef.current) {
      const elapsed = Math.floor((Date.now() - sessionStartRef.current) / 1000);
      const today = todayStr();
      const nextTotal = dailySeconds + elapsed;
